...
import { Plus, Edit, Trash2, IndianRupee, Search, Package, Tag, Flame, X, Leaf, Drumstick, Filter } from 'lucide-react';
...
export function MenuManagement() {
  ...
  const [menuItems, setMenuItems] = useState<MenuItem[]>([
    ...
    },
    {
      ...
      image: 'https://images.unsplash.com/photo-1586170128825-50c82cee6849?w=400',
      available: true,
      customizations: [
    ...
    },
    {
      ...
      image: 'https://images.unsplash.com/photo-1574440034581-4ab9e5eb2fc7?w=400',
      available: true,
      customizations: [
  ...
  const categories = [
    ...
    { id: 'appetizers', name: 'Appetizers', icon: Tag },
    { id: 'main-course', name: 'Main Course', icon: Flame },
    { id: 'desserts', name: 'Desserts', icon: Package },
    { id: 'beverages', name: 'Beverages', icon: Tag },
    { id: 'specials', name: 'Specials', icon: Flame }
  ];
    ...
    <div className="p-6 space-y-6">
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Select name="spiceLevel" defaultValue={editingItem?.spiceLevel || 'medium'}>
                      <SelectTrigger>
                ...
                <div className="border-t pt-4">
                  <div className="flex justify-between items-center mb-3">
                    <h3 className="font-semibold">Customizations (Add-ons with Checkbox, Spice Level, Options)</h3>
                  ...
                  <div className="space-y-3">
                    {customizations.map((custom) => (
                      <Card key={custom.id} className="p-3">
                        <div className="flex gap-2 items-start">
                          <div className="flex-1 space-y-3">
                            <div className="grid grid-cols-2 gap-2">
                              <Input
                                placeholder="Customization name (e.g., Add-ons, Spice Level)"
                                value={custom.name}
                                onChange={(e) => updateCustomization(custom.id, 'name', e.target.value)}
                              />
                              <Select 
                                value={custom.type} 
                                onValueChange={(value: any) => updateCustomization(custom.id, 'type', value)}
                              >
                                <SelectTrigger>
                                  <SelectValue placeholder="Type" />
                                </SelectTrigger>
                                <SelectContent>
                                  <SelectItem value="addon">Add-ons (Checkbox)</SelectItem>
                                  <SelectItem value="spicelevel">Spice Level</SelectItem>
                                  <SelectItem value="other">Other Options</SelectItem>
                                </SelectContent>
                              </Select>
                            </div>
                            <Input
                              placeholder="Options (comma-separated, e.g., Extra Cheese, Olives, Jalapeños)"
                              value={custom.options.join(', ')}
                              onChange={(e) => 
                                updateCustomization(custom.id, 'options', e.target.value.split(',').map(s => s.trim()).filter(s => s))
                              }
                            />
                            {custom.options.length > 0 && (
                              <div className="p-2 bg-muted/30 rounded-md">
                                <p className="text-xs text-muted-foreground mb-2">Preview Options:</p>
                                <div className="flex flex-wrap gap-2">
                                  {custom.options.map((option, idx) => (
                                    <Badge key={idx} variant="secondary" className="text-xs">
                                      {custom.type === 'addon' && '☑ '}{option}
                                    </Badge>
                                  ))}
                                </div>
                              </div>
                          ...
                          <Button
                            ...
                            variant="ghost"
                            size="sm"
                            onClick={() => removeCustomization(custom.id)}
                          >
      ...
      <Tabs defaultValue="items">
        <TabsList>
          ...
          <TabsTrigger value="combos">Combo Meals ({comboMeals.length})</TabsTrigger>
        </TabsList>
              ...
              <Card key={item.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                <div className="relative h-48">
                  ...
                  <div className="absolute top-2 right-2 flex gap-2 flex-wrap justify-end">
                    ...
                    <Badge variant={item.available ? 'default' : 'secondary'} className="bg-white/90">
              ...
              <Card key={combo.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                <div className="relative h-48">
                  ...
                  <div className="absolute top-2 right-2">
                    <Badge variant={combo.available ? 'default' : 'secondary'} className="bg-white/90">
